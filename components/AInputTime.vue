...
const hours = computed<number | undefined>({
  ...
  set(value) {
    ...
    if (localMinutes === undefined) {
      ...
      try {
        const now = new Date();
        const newMoment = moment(now).startOf('day');
        newMoment.set({
          hours: value,
          minutes: 0
        });
        model.value = newMoment;
      } catch (error) {
...
const minutes = computed<number | undefined>({
  ...
  set(value) {
    ...
    localMinutes = value;
    try {
      // Créer un nouveau moment en utilisant une date spécifique
      const now = new Date();
      const newMoment = moment(now).startOf('day');
      newMoment.set({
        hours: localHours,
...
<template>
  <div class="a-input-time">
    ...
    <ASelect mode="single" color="black" v-model="hours" :options="hoursOptions" name="hours" close-on-select
      placeholder="--">
    </ASelect>
    :
    <ASelect mode="single" color="black" v-model="minutes" :options="minutesOptions" name="minutes" close-on-select
      placeholder="--">
    </ASelect>
  </div>
</template>
